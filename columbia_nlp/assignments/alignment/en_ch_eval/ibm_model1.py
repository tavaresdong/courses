...
class IBM_Model1:
    ...
    def train_model(self, native_path, foreign_path, iter_count = 10):
        self.t = {}    
    ...
    def _align_sentence(self, na_tokens, fo_tokens, sentence_num, out_file):
        ...
        for i in range(0, len(fo_tokens)):
            ...
            for j in range(0, len(na_tokens)):
                ...
                print 'foreign: %s native: %s score: %f\r\n'%(fo, na, score)
                if score > max_score:
    ...
    def parse_align_text(self, path):
        infile = file(path)
        with open('test.align', 'w') as outfile:
            linenum = 1
            for line in infile:
                tokens = line.rstrip().split()
                for align in tokens:
...
if __name__ == '__main__':
    try:
        ...
        if train_model:
            ibm_model1.train_model('corpus.ch', 'corpus.en', 5)
            ibm_model1.save_model('ibm.tmodel2')
        else:
            ibm_model1.load_model('ibm.tmodel2')
            ibm_model1.parse_align_text('test.align.txt')
        ibm_model1.do_alignment('test.ch.txt', 'test.en.txt', 'ibm1.out')
        print 'Alignment done'
