...
HWSize_t WriteIndex(MemIndex mi, DocTable dt, const char *filename) {
  HWSize_t filesize = 0, dtres, mires, headerres;
  FILE *f;
  ...
  // MISSING:
  headerres = WriteHeader(f, dtres, mires);
  if (headerres == 0) {
    fclose(f);
    unlink(filename);
    return 0;
...
static HWSize_t WriteDocidDocnameFn(FILE *f,
  ...
  // MISSING:
  res = fwrite((char *) kv->value, 1, slen_ho, f);
  if (res != slen_ho)
    return 0;
...
static HWSize_t WriteDocPositionListFn(FILE *f,
  ...
  for (i = 0; i < num_pos_ho; i++) {
    ...
    // Buffer overflow here affected other values!
    LLIteratorGetPayload(it, &payload);
  
    // Truncate to 32 bits, then convert it to network order and write it out.
    // MISSING:
    position.position = (DocPositionOffset_t) ((intptr_t) payload);
    position.toDiskFormat();
    res = fwrite(&position, sizeof(position), 1, f);
    if (res != 1)
      return 0;

    total = total + sizeof(position);
    // Iterate to the next position.
    LLIteratorNext(it);
  }
  LLIteratorFree(it);

  if (total != sizeof(header) + sizeof(position) * num_pos_ho) abort();
