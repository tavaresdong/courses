...
int main(int argc, char **argv) {
  ...
  for (i = 0; i < 20000; i++) {
    ...
    Verify333(LookupHashTable(ht, FNVHashInt64((HTValue_t)i), &kv) == 1);
    Verify333(kv.key == FNVHashInt64((HTValue_t)i));
    //printf("%d\n", kv.key);
    //printf("%d\n", FNVHashInt64((HTValue_t)100));
  }

  // look up a few values
  // my add
  //printf("%d\n", LookupHashTable(ht, FNVHashInt64((HTValue_t)100), &kv));
