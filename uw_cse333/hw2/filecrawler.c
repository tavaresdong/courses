...
static void HandleDir(char *dirpath, DIR *d, DocTable *doctable,
  ...
  while (1) {
	...
	subdir = opendir(newfile);
	if(subdir == NULL) {
		continue;
	}
	else {
		// Recurse on this dir, using HandleDir
...
static void HandleFile(char *fpath, DocTable *doctable, MemIndex *index) {
  ...
  HTIteratorFree(it);
  //printf("done looping through the hash table.\n");
