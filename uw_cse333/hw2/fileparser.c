...
char *ReadFile(const char *filename, HWSize_t *size) {
  ...
  while (left_to_read > 0) {
	numread = read(fd, (void *)buf, left_to_read);
	if(numread == 0) break;

	else if(numread == -1) {
		// check the errno
		if(errno == EAGAIN) {
			perror("The read is blocked.");
			free(buf);
			return NULL;
		}
		else if(errno == EINTR) {
...
HashTable BuildWordHT(char *filename) {
  ...
  // failure.
  //printf("starting read filename\n");
  filecontent = (char *)ReadFile(filename, &filelen);
  if(filecontent == NULL || filelen == 0)
	return NULL;

  //printf("read filecontent done.\n");
  ...
  //printf("Verified, the file is all ASCII\n");
...
static void LoopAndInsert(HashTable tab, char *content) {
  ...
  while (1) {
	...
	// curptr refers to a separator
	if(*curptr == '\0')
		endhit = 1;
	*curptr = '\0';
	AddToHashtable(tab, wordstart, wspos);
...
static void AddToHashtable(HashTable tab, char *word, DocPositionOffset_t pos) {
  ...
  if (retval == 1) {
   ...
   // printf("exist case done\n");
    ...
    // Big bug happen here, strcpy will copy null-terminator('\0'), so we need
    // to allocate one more position than what's returned by strlen.
    strcpy(wp->word, word);
